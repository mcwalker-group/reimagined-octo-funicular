...
def leader_motif(fimo_file):
    ...
    for line in fimo:
        if line[0] != "#":
            if line.split()[1] == "MEME-1" and -1*np.log10(float(line.split()[7])) > 5:
            ...
            elif line.split()[1] == "MEME-10" and -1*np.log10(float(line.split()[7])) > 5:
                meme10[line.split()[2]] = (line.split()[3], line.split()[4])
    fimo.close()
    return [meme1, meme2, meme3, meme10]
...
def cut_pep(pep, lead_motifs):
    ...
    for meme in lead_motifs:
        if acc in meme:
            stops.append(int(meme[acc][1]))
    stops.sort()
    l_stop = stops[-1]

    regex1 = re.search('(.[S|T].{2,7}C)', seq[l_stop:])
    regex2 = re.search('(G[G|A])', seq[l_stop:])
